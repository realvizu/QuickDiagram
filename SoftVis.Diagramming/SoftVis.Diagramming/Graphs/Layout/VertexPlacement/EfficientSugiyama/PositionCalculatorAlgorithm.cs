...
namespace Codartis.SoftVis.Graphs.Layout.VertexPlacement.EfficientSugiyama
{
    internal class PositionCalculatorAlgorithm : IAlgorithm
    {
        ...
        public Dictionary<ISized, Point2D> VertexCenters { get; private set; }
            ...
            List<SugiVertex> isolatedVertices)
        {
            ...
            VertexCenters = new Dictionary<ISized, Point2D>();
        }
        ...
        private static Dictionary<ISized, Point2D> GetVertexCenters(SugiGraph sugiGraph)
        {
            var vertexCenters = new Dictionary<ISized, Point2D>();

            foreach (var sugiVertex in sugiGraph.Vertices.Where(i => i.Type == SugiVertexType.Original))
            {
                var newPosition = new Point2D(sugiVertex.HorizontalPosition, sugiVertex.VerticalPosition);
