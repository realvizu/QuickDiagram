...
namespace Codartis.SoftVis.Graphs.Layout.VertexPlacement.EfficientSugiyama
{
    ...
    /// </summary>
    internal class CrossingMinimizationAlgorithm : IAlgorithm
    {
        private readonly SugiGraph _sugiGraph;
        private readonly Layers _layers;
        private readonly AlternatingLayer[] _alternatingLayers;
        private readonly int[] _crossCountsPerLayer;
        private readonly Random _random;

        public IMutableBidirectionalGraph<Data, IEdge<Data>> SparseCompactionGraph { get; }
        public IList<Edge<Data>>[] SparseCompactionByLayerBackup { get; }

        internal CrossingMinimizationAlgorithm(SugiGraph sugiGraph, Layers layers)
        {
            _sugiGraph = sugiGraph;
            _layers = layers;
            _alternatingLayers = new AlternatingLayer[layers.Count];
            _crossCountsPerLayer = Enumerable.Repeat(int.MaxValue, layers.Count).ToArray();

            SparseCompactionGraph = new BidirectionalGraph<Data, IEdge<Data>>();
            SparseCompactionByLayerBackup = new IList<Edge<Data>>[layers.Count];
        ...
        private int Sweeping(int startLayerIndex, int endLayerIndex, int step, bool enableSameMeasureOptimization, out bool changed, ref int phase)
        {
            ...
            for (int i = startLayerIndex; i != endLayerIndex; i += step)
            {
                ...
                int prevCrossCount = _crossCountsPerLayer[ci];
                ...
                if (_alternatingLayers[i + step] != null)
                {
                    ...
                    _crossCountsPerLayer[ci] = prevCrossCount;
                }
                ...
                if (crossCount < prevCrossCount || phase == 2 || changed)
                {
                    ...
                    _crossCountsPerLayer[i] = crossCount;
        ...
        {
            var ignorableVertexType = straightSweep ? SugiVertexType.QVertex : SugiVertexType.PVertex;
        ...
        {
            var _sparseCompationGraphEdgesOfLayer = SparseCompactionByLayerBackup[layerIndex];
            if (_sparseCompationGraphEdgesOfLayer != null)
            {
                foreach (var edge in _sparseCompationGraphEdgesOfLayer)
                    SparseCompactionGraph.RemoveEdge(edge);
            }
            ...
            for (int i = 1; i < nextAlternatingLayer.Count; i += 2)
            {
                ...
                if (prevContainer != null && prevContainer.Count > 0)
                {
                    ...
                    SparseCompactionGraph.AddVerticesAndEdge(edge);
                }
                ...
                if (vertex != null && !SparseCompactionGraph.ContainsVertex(vertex))
                    SparseCompactionGraph.AddVertex(vertex);
                prevVertex = vertex;
            }
            SparseCompactionByLayerBackup[layerIndex] = _sparseCompationGraphEdgesOfLayer;
        }
        ...
        private static void ReinsertVerticesIntoLayer(AlternatingLayer layer, HashSet<SugiVertex> vertexSet, IList<SugiVertex> vertexList)
        {
            int reinsertIndex = 0;
        ...
        private IList<SugiEdge> FindRealEdges(AlternatingLayer topLayer)
        {
            ...
            {
                var vertex = item as SugiVertex;
                if (vertex == null || vertex.Type == SugiVertexType.PVertex)
                    continue;

                foreach (var edge in _sugiGraph.OutEdges(vertex))
                    realEdges.Add(edge);
            }
            return realEdges;
        }
        ...
        private IList<CrossCounterPair> FindVirtualEdgePairs(AlternatingLayer topLayer, AlternatingLayer bottomLayer)
        {
            ...
            Queue<VertexGroup> firstLayerQueue = GetContainerLikeItems(topLayer, SugiVertexType.PVertex);
            Queue<VertexGroup> secondLayerQueue = GetContainerLikeItems(bottomLayer, SugiVertexType.QVertex);
            VertexGroup vg1, vg2;
            vg1 = new VertexGroup();
            vg2 = new VertexGroup();
        ...
        private static Queue<VertexGroup> GetContainerLikeItems(AlternatingLayer alternatingLayer, SugiVertexType containerLikeVertexType)
        {
        ...
        private void PlaceQVertices(AlternatingLayer alternatingLayer, Layer nextLayer, bool straightSweep)
        {
            var type = straightSweep ? SugiVertexType.QVertex : SugiVertexType.PVertex;
        ...
        {
            var type = straightSweep ? SugiVertexType.PVertex : SugiVertexType.QVertex;
        ...
        private AlternatingLayer InitialOrderingOfNextLayer(AlternatingLayer alternatingLayer, Layer nextLayer, bool straightSweep)
        {
        ...
        private void AssignMeasuresOnNextLayer(Layer layer, bool straightSweep)
        {
            ...
            {
                if ((straightSweep && vertex.Type == SugiVertexType.QVertex)
                    || (!straightSweep && vertex.Type == SugiVertexType.PVertex))
                    continue;

                var edges = straightSweep ? _sugiGraph.InEdges(vertex) : _sugiGraph.OutEdges(vertex);
