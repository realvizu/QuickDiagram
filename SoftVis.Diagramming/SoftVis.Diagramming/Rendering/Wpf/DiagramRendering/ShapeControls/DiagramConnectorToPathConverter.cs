...
namespace Codartis.SoftVis.Rendering.Wpf.DiagramRendering.ShapeControls
{
    public class DiagramConnectorToPathConverter : IMultiValueConverter
    {
        ...
        {
            Debug.Assert(values != null && values.Length == 3, "DiagramConnectorToPathConverter should have these parameters: SourceRect, TargetRect, RoutePoints.");
            Debug.Assert(values[0] != DependencyProperty.UnsetValue, "SourceRect parameter is mandatory.");
            Debug.Assert(values[1] != DependencyProperty.UnsetValue, "TargetRect parameter is mandatory.");

            var sourceRect = (Rect)values[0];
            var targetRect = (Rect)values[1];
            var routeInformation = (Point[])values[2];
            ...
            var boundingRect = new[] { sourceRect, targetRect }.Union();
            routeInformation = MakePositionsRelativeToBoundingRect(routeInformation, boundingRect);
